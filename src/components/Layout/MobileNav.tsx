    ...
    >
      <div className="grid grid-cols-5 items-stretch gap-1 px-2 py-1.5">
        {NAV_ITEMS.map((item) => {
          ...
          const linkClasses = `flex min-w-0 flex-col items-center justify-center gap-1 rounded-xl px-1 py-1.5 min-h-[52px] transition-all duration-200 active:scale-95 tv-focusable tv-touch-target ${
            isActive ? 'text-white bg-primary/10' : 'text-gray-500 hover:text-gray-300'
          }`
          const iconClasses = `w-5 h-5 ${isActive ? 'text-primary' : ''}`
          const labelClasses = `max-w-full truncate text-[11px] leading-none ${isActive ? 'font-semibold' : ''}`
