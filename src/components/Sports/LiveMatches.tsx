  ...
  useEffect(() => {
    async function fetchMatches() {
      try {
        ...
        setFetchError(`Unable to load ${variant === 'upcoming' ? 'upcoming' : 'live'} matches.`)
        toast.error(`Match load failed: ${message}`)
        setLoading(false)
      }
    }
    ...
    return (
      <div className="space-y-4">
        <div className="glass rounded-lg p-6 text-center space-y-3">
          <p className="text-sm font-semibold text-primary">Unable to load matches</p>
